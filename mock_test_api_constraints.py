...
from src.pyhearingai.infrastructure.adapters.size_aware_audio_converter import SizeAwareFFmpegConverter
...
def log_event(event):
    ...
    if isinstance(event, AudioValidationEvent):
        ...
        if event.is_successful:
            ...
            logger.info("✅ Conversion successful: %s -> %s", 
                       event.source_path.name, event.target_path.name)
            logger.info("   Original size: %.2f MB", 
                       event.original_size / 1_000_000)
            logger.info("   Converted size: %.2f MB", 
                       event.converted_size / 1_000_000)
        ...
        logger.warning("   Target size: %.2f MB", 
                      event.target_size_bytes / 1_000_000)
        logger.warning("   Best achieved: %.2f MB", 
                      event.best_achieved_size / 1_000_000)
...
def test_api_constraints(audio_path):
    ...
    providers = [
        ...
        ApiProvider.ASSEMBLY_AI,
        ApiProvider.GOOGLE_SPEECH
    ]
    ...
    for provider in providers:
        ...
        logger.info("   Max file size: %.2f MB", 
                   limits.max_file_size_bytes / 1_000_000)
        logger.info("   Max duration: %.1f minutes if applicable (0 means no limit)", 
        ...
        if is_valid:
            ...
            try:
                output_path = TEMP_DIR / f"{audio_path.stem}_{provider.name.lower()}{audio_path.suffix}"
    ...
    for name, spec in quality_specs:
        ...
        try:
            output_path = TEMP_DIR / f"{audio_path.stem}_{name.lower().replace(' ', '_')}.{spec.format.value}"
            ...
            logger.info("   Size: %.2f MB", 
                       converted_metadata.get('size_bytes', 0) / 1_000_000)
            logger.info("   Duration: %.2f seconds", 
                       converted_metadata.get('duration', 0))
            logger.info("   Sample rate: %s Hz", 
                       converted_metadata.get('sample_rate', 0))
            logger.info("   Channels: %s", 
                       converted_metadata.get('channels', 0))
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "audio_path", nargs="?", default="test data/short_conversation.m4a",
