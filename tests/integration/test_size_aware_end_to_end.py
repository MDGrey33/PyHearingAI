...
def run_transcription_command(
    audio_file, output_dir, start_time=0, end_time=None, max_chunk_size=None
):
...
def test_short_conversation_end_to_end(temp_output_dir):
    ...
    result = run_transcription_command(
        SHORT_AUDIO, temp_output_dir, start_time=0, end_time=240  # 4 minutes
    )
...
def test_long_conversation_end_to_end(temp_output_dir):
    ...
    result = run_transcription_command(
        LONG_AUDIO, temp_output_dir, max_chunk_size=300  # 5-minute chunks
    )
