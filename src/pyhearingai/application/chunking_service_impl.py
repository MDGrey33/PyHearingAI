...
from pyhearingai.core.ports import ChunkingService, AudioFormatService
...
class ChunkingServiceImpl(ChunkingService):
    ...
    def create_audio_chunks(
        ...
        format_str = quality_spec.format.value if hasattr(quality_spec.format, 'value') else quality_spec.format
        ...
        for i, (start_time, end_time) in enumerate(chunk_boundaries):
            ...
            try:
                ...
                logger.debug(f"Creating chunk {i+1}/{len(chunk_boundaries)}: {start_time}-{end_time}")
                ...
                if not is_valid:
                    ...
                    if adjusted_spec:
                        ...
                        try:
                            logger.info(f"Attempting direct size-constrained conversion for chunk {i+1}")
                            converted_path, metadata = self.audio_converter.convert_with_size_constraint(
                                extracted_path, 
                            ...
                            if is_valid:
                                logger.info(f"Successfully reduced size of chunk {i+1} with direct conversion")
                                chunk_paths.append(converted_path)
                            else:
                                logger.warning(
        ...
        EventPublisher.publish(ChunkingEvent(
            source_path=audio_path,
            job_id=job_id,
            chunk_count=len(chunk_paths),
            chunk_duration=chunk_boundaries[0][1] - chunk_boundaries[0][0] if chunk_boundaries else 0,
