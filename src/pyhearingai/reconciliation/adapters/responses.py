...
class TokenCounter:
    ...
    def estimate_batch_size(
        self, segments: List[Any], sample_size: int = 5, max_tokens: int = 6000
    ) -> int:
        """
...
class BatchProcessor:
    ...
    def create_batches(
        ...
        chunk_ids = sorted(
            set(list(diarization_segments.keys()) + list(transcription_segments.keys()))
        )
        if not chunk_ids:
        ...
        for chunk_id in chunk_ids:
            ...
            serialized_diarization = self._serialize_segments(
                diarization_segments.get(chunk_id, [])
            )
            serialized_transcription = self._serialize_segments(
                transcription_segments.get(chunk_id, [])
            )
            ...
            chunk_segment_transcriptions = {
                seg_id: text
                for seg_id, text in segment_transcriptions.items()
            ...
            if chunk_tokens > self.max_tokens:
                logger.warning(
                    f"Chunk {chunk_id} exceeds token limit ({chunk_tokens} tokens). "
    ...
    def format_batch_prompt(
        self, batch: Dict[str, Any], batch_index: int, total_batches: int, job: ProcessingJob
    ) -> str:
        """
        ...
        if total_batches > 1:
            if batch_index == 0:
                batch_context += (
                    " This is the first batch, so there are no previous segments to consider."
                )
            else:
                batch_context += (
                    " Please maintain consistent speaker identities from previous batches."
                )

            if not is_final_batch:
                batch_context += (
                    " This is not the final batch, so the audio continues beyond what's shown here."
                )
...
class ResultAggregator:
    ...
    def process_responses(
        self, responses: List[str], overlap_threshold: float = 0.5
    ) -> List[Segment]:
        """
    ...
    def _merge_overlapping_segments(
        self, segments: List[Segment], threshold: float = 0.5
    ) -> List[Segment]:
        """
        Merge overlapping segments from the same speaker.
        ...
        for current in sorted_segments[1:]:
            ...
            if (
                ...
                and current.start - previous.end <= threshold
            ):
...
class ResponsesReconciliationAdapter(BaseReconciliationAdapter):
    ...
    def reconcile(
        ...
        for i, batch in enumerate(batches):
            try:
                ...
                prompt = self.batch_processor.format_batch_prompt(batch, i, len(batches), job)

                # Create the request
                response = self._process_batch(prompt, previous_response_id, options)
    ...
    def _process_batch(
        self, prompt: str, previous_response_id: Optional[str], options: Optional[Dict[str, Any]]
    ) -> Any:
        """
                ...
                if previous_response_id:
                    ...
                    response = self.client.chat.completions.create(
                        ...
                        messages=[
                            {
                                "role": "system",
                                "content": "You are an expert audio transcription assistant.",
                            },
