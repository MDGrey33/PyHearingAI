...
class SizeAwareFFmpegConverter(FFmpegAudioConverter, SizeAwareAudioConverter):
        ...
        **kwargs
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result_size <= max_size_bytes:
                ...
                logger.info(f"Initial conversion meets size constraint ({result_size} <= {max_size_bytes})")
                ...
                EventPublisher.publish(AudioConversionEvent(
                    ...
                    quality_spec=AudioQualitySpecification(
                        ...
                        format=AudioFormat(target_format) if hasattr(AudioFormat, target_format.upper()) else None,
            ...
            if target_format != "mp3":
                logger.info(
                    "All format-specific adjustments failed. "
                    "Trying MP3 format as last resort."
                )
            ...
            EventPublisher.publish(AudioSizeExceededEvent(
                source_path=audio_path,
    ...
    def convert_with_quality_spec(
        ...
        **kwargs
        ...
        format_str = quality_spec.format.value if isinstance(quality_spec.format, AudioFormat) else quality_spec.format
        codec_str = quality_spec.codec.value if isinstance(quality_spec.codec, AudioCodec) else quality_spec.codec
