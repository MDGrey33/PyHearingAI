...
def register_output_formatter(format_name: str) -> Callable[[Type[OutputFormatter]], Type[OutputFormatter]]:
    """
...
def get_transcriber(name: str) -> Transcriber:
    ...
    if name not in _transcribers:
        raise ValueError(f"Transcriber '{name}' not found. Available transcribers: {list(_transcribers.keys())}")
...
def get_diarizer(name: str) -> Diarizer:
    ...
    if name not in _diarizers:
        raise ValueError(f"Diarizer '{name}' not found. Available diarizers: {list(_diarizers.keys())}")
...
def get_output_formatter(format_name: str) -> OutputFormatter:
    ...
    if format_name not in _output_formatters:
        raise ValueError(f"Output formatter '{format_name}' not found. Available formatters: {list(_output_formatters.keys())}")
